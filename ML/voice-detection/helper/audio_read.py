...
def audio_file(filelocation, pre_emphasis: float = 0.97, top_db: int = 30) -> np.ndarray:
    """
    ...
    Args:
        filelocation  (str/file-like) : Path to the audio file or a BytesIO object.
    ...
    """
    is_file_like = hasattr(filelocation, 'read')
    temp_path = None

    if is_file_like:
        fd, temp_path = tempfile.mkstemp(suffix=".webm")
        with os.fdopen(fd, 'wb') as f:
            if hasattr(filelocation, 'seek'):
                filelocation.seek(0)
            f.write(filelocation.read())
        path_to_load = temp_path
    else:
        path_to_load = filelocation

    try:
        # 1. Load audio as 16 kHz mono
        audio, sr = librosa.load(path_to_load, sr=16000, mono=True)
    finally:
        if is_file_like and temp_path and os.path.exists(temp_path):
            try:
                os.remove(temp_path)
            except Exception:
                pass
