...
def voice_to_embedding(voicefile, chunk_size_sec=5.0, hop_size_sec=2.5, min_duration=None):
    """
    ...
    Args:
        ...
        min_duration   (float) : Minimum duration in seconds. Defaults to MIN_AUDIO_DURATION_SEC.
    ...
    required_duration = min_duration if min_duration is not None else MIN_AUDIO_DURATION_SEC
    
    if duration_sec < required_duration:
        raise ValueError(
            f"Audio file '{voicefile}' is too short: "
            f"{duration_sec:.2f}s (minimum required: {required_duration}s). "
